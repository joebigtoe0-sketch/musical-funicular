...
const { getAnalyticsConsentStub } = require("./analytics");
...
const PAGE_META = {
  ...
  "tietosuoja.html": {
    path: "/tietosuoja.html",
    title: "Tietosuojaseloste | Sairas Media",
    description: "Sairas Median tietosuojaseloste: henkilötietojen käsittely, evästeet ja oikeutesi.",
    ogType: "website",
    keywords: "tietosuoja, evästeet, GDPR, Sairas Media",
    jsonLd: "page",
  },
...
function getCalendlyConfigScript() {
  const url = (process.env.SAIRAS_CALENDLY_URL || "").trim();
  if (!url.startsWith("https://")) return "";
  return `\n  <script>window.SAIRAS_CALENDLY_URL = "${escapeAttr(url)}";</script>`;
}
...
function getFaviconTags() {
  return `${getGoogleVerificationTag()}${getCalendlyConfigScript()}${getAnalyticsConsentStub()}
